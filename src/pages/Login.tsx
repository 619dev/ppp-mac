  ...
  return (
    <div className="login-page">
      ...
      <div className="login-form-panel">
        <div className="login-card">
          <h2 className="login-title">{isRegister ? t('auth.register') : t('auth.login')}</h2>
              ...
              <div style={{
                ...
                {showAddForm && (
                  <div style={{
                    ...
                    <input className="input" type="text" placeholder={t('proxy.name_placeholder')} value={formName} onChange={e => setFormName(e.target.value)} style={{ fontSize: 13 }} />
                    ...
                    <div style={{ display: 'flex', gap: 4 }}>
                      <input className="input" type="text" placeholder={t('proxy.host')} value={formHost} onChange={e => setFormHost(e.target.value)} style={{ flex: 3, fontSize: 13 }} />
                      <input className="input" type="text" inputMode="numeric" placeholder={t('proxy.port')} value={formPort} onChange={e => setFormPort(e.target.value.replace(/\D/g, ''))} style={{ flex: 1, fontSize: 13 }} />
                    </div>
                    <div style={{ display: 'flex', gap: 4 }}>
                      <input className="input" type="text" autoComplete="off" placeholder={`${t('proxy.username')} (${t('proxy.optional')})`} value={formUser} onChange={e => setFormUser(e.target.value)} style={{ flex: 1, fontSize: 12 }} />
                      <input className="input" type="password" autoComplete="off" placeholder={`${t('proxy.password')} (${t('proxy.optional')})`} value={formPass} onChange={e => setFormPass(e.target.value)} style={{ flex: 1, fontSize: 12 }} />
                    </div>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button type="button" className="btn btn-primary btn-full btn-sm" onClick={handleAddProxy} disabled={!formHost || !formPort} style={{ flex: 2, borderRadius: 8, fontSize: 12 }}>{t('common.save')}</button>
                      <button type="button" className="btn btn-full btn-sm" onClick={() => { setShowAddForm(false); resetProxyForm() }} style={{ flex: 1, borderRadius: 8, fontSize: 12 }}>{t('common.cancel')}</button>
                    </div>
                  </div>
                )}
              </div>
